...
export default function Button<T extends unknown[]>({
  ...
  const baseClasses = `grid items-center justify-center rounded-md bg-gradient-to-r hover:bg-gradient-to-l bg-[length:200%_200%] bg-[position:0%_50%] hover:bg-[position:95%_50%] transition-[background-position] duration-500 ease-out font-medium shadow-sm text-white ${isActive || isLoading ? 'cursor-not-allowed' : 'cursor-pointer'
    }`;
  ...
  return (
    ...
    >
      ...
      <span
        className={`col-start-1 row-start-1 flex items-center justify-center gap-2 p-0.5! ${isActive || isLoading ? 'invisible' : 'visible'
          }`}
      >
        {icon}
        {text && <span>{text}</span>}
